      ...
      <nav className="min-h-0 flex-1 overflow-y-auto divide-y divide-border/50">
        {filtered.length === 0 ? (
          <p className="text-muted-foreground px-4 py-6 text-xs text-center">
            {rows.length === 0 ? 'No conversations active yet.' : 'No matches found.'}
          </p>
        ) : (
          <ul>
            {filtered.map((row) => {
              ...
              const totalUnread = row.unreadCustomerCount + row.unreadVendorCount
              const color = avatarColor(row.customerName)
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline">
                        ...
                        <span className="text-xs text-muted-foreground/60 shrink-0 ml-2">
                          {row.lastMessageAt ? relativeTime(row.lastMessageAt) : relativeTime(row.pickupAt)}
                        </span>
                      </div>
