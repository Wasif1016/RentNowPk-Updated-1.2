...
import { updateAdminName, type AdminNameResult } from '@/lib/actions/admin-profile'
...
async function action(
  ...
  return updateAdminName(_prev, formData)
}

export function AdminNameCard({ initialFullName }: { initialFullName: string }) {
  ...
  return (
    ...
    >
      <form action={formAction} noValidate className="space-y-4">
        <div>
          <label htmlFor="admin-fullName" className="block text-sm font-medium text-foreground mb-1.5">
            Full name
          </label>
          <Input
            id="admin-fullName"
