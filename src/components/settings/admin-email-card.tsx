...
import { requestAdminEmailChange, type AdminEmailResult } from '@/lib/actions/admin-profile'
...
async function action(
  ...
  return requestAdminEmailChange(_prev, formData)
}

export function AdminEmailCard({ currentEmail }: { currentEmail: string }) {
    ...
    >
      {showSuccess ? (
        ...
        <form action={formAction} noValidate className="space-y-4">
          <div>
            <label htmlFor="admin-new-email" className="block text-sm font-medium text-foreground mb-1.5">
              New email address
            </label>
            <Input
              id="admin-new-email"
