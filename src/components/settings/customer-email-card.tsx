...
import { requestCustomerEmailChange, type CustomerEmailResult } from '@/lib/actions/customer-profile'
...
async function action(
  ...
  return requestCustomerEmailChange(_prev, formData)
}

export function CustomerEmailCard({ currentEmail }: { currentEmail: string }) {
      ...
      {showSuccess ? (
        <div className="rounded-lg bg-green-50 border border-green-200 p-4">
          <p className="text-sm text-green-700">
            Check the inbox of the address you entered and click the confirmation link.
          </p>
        </div>
      ) : (
        <form action={formAction} noValidate className="space-y-4">
          <div>
            <label htmlFor="customer-new-email" className="block text-sm font-medium text-foreground mb-1.5">
              New email address
            </label>
            <Input
